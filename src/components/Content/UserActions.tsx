...
const UserActions: React.FC<UserActionsProps> = ({
  ...
  const showConfirmModal = (title: string, content: string, onOk: () => void) => {
    Modal.confirm({
      title,
      content: <div style={styles.modalContent}>{content}</div>,
      onOk,
      okText: 'Да',
      cancelText: 'Нет',
      width: 400,
  ...
  return (
    <div style={styles.container}>
      <Button
        onClick={() => navigate(`${id}`)}
        style={{...styles.button.base, ...styles.button.profile}}
        icon={<User size={20} />}
      />
      <Button
        onClick={deleteHandler}
        style={{...styles.button.base, ...styles.button.delete}}
        icon={<Trash2 size={20} />}
      />
      <Button
        onClick={blockHandler}
        style={{...styles.button.base, ...styles.button.block}}
        icon={isBlocked ? <Unlock size={20} /> : <Lock size={20} />}
      />
      <Button
        onClick={rolesHandler}
        style={{...styles.button.base, ...styles.button.admin}}
